  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      try {
        ...
        L.marker(buenosAiresCenter).addTo(map).bindPopup('Buenos Aires');
  ...
  return <div className="min-h-screen bg-background" dir="ltr">
      ...
      <main className="px-4 lg:px-6 py-4 lg:py-6 space-y-6 lg:space-y-8 pb-20 lg:pb-8 w-full">
        ...
        <section className="bg-card/30 backdrop-blur-sm rounded-xl p-3 lg:p-5 border border-border/20 shadow-sm">
          ...
          scrollBehavior: 'smooth'
              ...
              {meetupEvents.map((event, index) => <ScrollAnimatedCard key={`meetup-${event.id}`} index={index}>
                  <UniformCard id={event.id} image={event.image_url || communityEvent} video={(event as any).video_url} title={event.title} subtitle={event.location || 'Tel Aviv'} price={event.price} date={getRelativeDay(event.date)} type="event" uploader={event.uploader} onProfileClick={userId => navigate(`/profile/${userId}`)} onClick={() => handleMeetupClick({
              id: event.id,
              title: event.title,
              description: event.description || event.title,
        ...
        <section className="bg-card/30 backdrop-blur-sm rounded-xl p-3 lg:p-5 border border-border/20 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold text-foreground relative">
            <span className="relative z-10 bg-gradient-to-r from-foreground via-foreground to-muted-foreground bg-clip-text text-transparent drop-shadow-sm">Events Happening Around</span>
              <div className="absolute inset-0 bg-gradient-to-r from-primary/10 via-primary/8 to-primary/5 blur-sm -z-10 transform translate-x-0.5 translate-y-0.5 rounded-md"></div>
            </h2>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => {
          ...
          scrollBehavior: 'smooth'
              ...
              {realEvents.slice(0, 3).map((event, index) => <ScrollAnimatedCard key={`event-${event.id}`} index={index}>
        ...
        <section className="bg-card/30 backdrop-blur-sm rounded-xl p-3 lg:p-5 border border-border/20 shadow-sm">
          ...
          <div className="flex overflow-x-auto gap-5 pb-3 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-muted-foreground/20 hover:scrollbar-thumb-muted-foreground/40" dir="ltr" style={{
          scrollBehavior: 'smooth'
        }}>
      ...
      {/* Map Overlay */}
      {isMapOpen && <div className="fixed inset-0 bg-background z-50 flex flex-col">
          {/* Map Header */}
          <div className="flex items-center justify-between p-4 border-b border-border/20 bg-background/95 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              <Button variant="ghost" size="icon" onClick={handleMapToggle} className="h-8 w-8">
