...
export const useFriendsPictureGalleries = () => {
  ...
  const fetchPictureGalleries = async () => {
    if (!user) return;
  ...
  const createPictureGallery = async (title?: string, images: string[] = []) => {
  ...
  return {
    ...
    deleteGallery,
    refreshGalleries: fetchPictureGalleries
  };
};