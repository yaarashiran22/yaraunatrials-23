...
const CommunityCard = ({ community, onClick, onUpdate, onDelete }: CommunityCardProps) => {
  ...
  return (
    <div 
      className="bg-card rounded-lg border border-border shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer overflow-hidden w-full aspect-[2/3] flex flex-col"
        ...
        {community.creator && (
          <div className="flex items-center gap-2 mb-3 px-2">
            <Avatar className="w-5 h-5">
              <AvatarImage src={community.creator.profile_image_url || ''} />
              <AvatarFallback className="text-xs bg-muted">
                {community.creator.name?.charAt(0)?.toUpperCase() || 'U'}
              </AvatarFallback>
            </Avatar>
            <span className="text-xs text-muted-foreground line-clamp-1">
              by {community.creator.name || 'Anonymous'}
            </span>
          </div>
        )}
