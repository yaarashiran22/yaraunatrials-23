...
const EventVerticalPopup = ({
  ...
  return <div className="fixed inset-0 z-[9999] bg-black/50 flex items-center justify-center p-4" ref={containerRef}>
      ...
      <div ref={contentRef} className={`bg-background rounded-2xl w-full max-w-sm ${isMobile ? 'max-h-[95vh]' : 'max-h-[85vh]'} overflow-y-auto mx-4 relative transition-transform duration-200 ${isDragging ? 'cursor-grabbing' : 'cursor-grab'} event-vertical-popup`} style={{
      ...
      scrollBehavior: 'smooth'
        ...
        <div className="flex items-center justify-between p-4 border-b bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-950/30 dark:to-blue-950/30">
          ...
          <div className="flex items-center gap-2">
            {allEvents.length > 1 && <div className="text-sm text-muted-foreground bg-white/80 dark:bg-black/20 px-3 py-1 rounded-full">
        ...
        {allEvents.length > 1 && <div className="flex justify-center py-2 border-b bg-gradient-to-r from-green-50/50 to-blue-50/50 dark:from-green-950/20 dark:to-blue-950/20">
        ...
        {allEvents.length > 1 && <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-10">
            {allEvents.map((_, index) => <div key={index} className={`w-2 h-8 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-primary shadow-sm scale-110' : 'bg-muted-foreground/20'}`} />)}
          </div>}
        ...
        <div className={`${isMobile ? 'p-3' : 'p-4'}`}>
          ...
          <div className={`relative ${isMobile ? 'mb-4' : 'mb-6'}`}>
            <div className="border-4 border-gradient-to-r from-green-400 to-blue-400 rounded-2xl overflow-hidden">
              <img src={displayEvent.image} alt={displayEvent.title} className={`w-full ${isMobile ? 'h-48' : 'h-64'} object-cover`} />
            </div>
          ...
          <div className={`${isMobile ? 'space-y-3' : 'space-y-4'}`}>
            <div className="text-center">
              ...
              {displayEvent.price && displayEvent.price !== 'Free' && <p className={`${isMobile ? 'text-base' : 'text-lg'} font-semibold text-primary`}>{displayEvent.price}</p>}
              {(!displayEvent.price || displayEvent.price === 'Free') && <p className={`${isMobile ? 'text-base' : 'text-lg'} font-semibold text-green-600 dark:text-green-400`}>Free Event</p>}
            </div>
            
            {displayEvent.description && <p className={`text-foreground leading-relaxed text-center ${isMobile ? 'text-xs' : 'text-sm'}`}>
            ...
            <div className={`${isMobile ? 'space-y-2' : 'space-y-3'} ${isMobile ? 'p-3' : 'p-4'} bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-950/30 dark:to-blue-950/30 rounded-lg border border-green-200 dark:border-blue-800`}>
              {displayEvent.location && <div className={`flex items-center gap-3 ${isMobile ? 'text-xs' : 'text-sm'} font-medium text-foreground`}>
              ...
              {displayEvent.date && <div className={`flex items-center gap-3 ${isMobile ? 'text-xs' : 'text-sm'} font-medium text-foreground`}>
            ...
            {/* Organizer Info */}
            {displayEvent.organizer && <div className={`flex items-center gap-3 ${isMobile ? 'p-2' : 'p-3'} bg-muted/30 rounded-lg cursor-pointer hover:bg-muted/50 transition-colors`} onClick={handleViewProfile}>
                <img src={displayEvent.organizer.image} alt={displayEvent.organizer.name} className={`${isMobile ? 'w-8 h-8' : 'w-10 h-10'} rounded-full object-cover`} />
                <div className="flex-1">
                  <p className={`font-semibold text-foreground ${isMobile ? 'text-xs' : 'text-sm'}`}>{displayEvent.organizer.name}</p>
                  {displayEvent.organizer.location && <div className={`flex items-center gap-1 ${isMobile ? 'text-xs' : 'text-xs'} text-muted-foreground`}>
